...
class Report_BPHTB_model extends CI_Model {
  ...
  { 
    ...
    {   
        $this->db->select("tc.*, tt.users_name, twp.nama_wajib_pajak")
          ->from("tb_backend_trx_bphtb tc")
          ->join("tb_backend_users tt", "tt.id_backend_users = tc.id_backend_users")
          ->join("tb_backend_wajib_pajak_bphtb twp", "twp.idtb_backend_wajib_pajak_bphtb = tc.id_backend_wp");
        $this->db->where("tc.status = 'true'");
