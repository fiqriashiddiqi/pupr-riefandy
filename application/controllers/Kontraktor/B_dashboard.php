...
class B_dashboard extends CI_Controller {
	...
	public function index()
	{
		...
		$data['brd_title_url'] = site_url('Kontraktor/B_dashboard');
		...
		$this->load->view('backend-web/dashboard', $data);
		$this->load->view('backend-web/partial/footer');

	}

    public function input_no()
	{	
		$users_username = $this->session->userdata('users_username');
		$no_kontrak = $this->input->post('no_kontrak');
		
			if ($no_kontrak = 12345678){
				redirect(base_url().'Kontraktor/B_dashboard');

			}else{
				$this->session->set_flashdata('alert_error', 'Nomor Kontrak Tidak Terdaftar!');
