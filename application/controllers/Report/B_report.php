...
class B_report extends CI_Controller {
	...
	public function index()
	{
        ...
        if($access_groups==1){
    		...
    		$data['title'] = "Laporan";
    		$data['brd_title_main'] = "Laporan";
    		$data['brd_title_url'] = site_url('Report/B_report');
    		...
    		$data['excel_url'] = site_url('Report/B_report/excel_laporan');
            ...
            $data['data_laporan_user'] = $this->report_model->get_laporan_by_id_users(null,@$id_backend_users);
    		$data['data_laporan'] = $this->report_model->get_laporan_list_report_admin(null,@$id_backend_users,@$start_date,@$end_date);
            ...
            $data['title'] = "Laporan Manajemen";
            $data['brd_title_main'] = "Laporan";
            $data['brd_title_url'] = site_url('Report/B_report');

            //$data['create_url'] = site_url('General_Ledger/B_journal/create_journal');
            //$data['info_url'] = site_url('General_Ledger/B_journal/update_journal');
            $data['excel_url'] = site_url('Report/B_report/excel_laporan_manajemen');
            ...
            $data['brd_title_main'] = "Laporan Petugas";
            $data['brd_title_url'] = site_url('Report/B_report');

            //$data['create_url'] = site_url('General_Ledger/B_journal/create_journal');
            //$data['info_url'] = site_url('General_Ledger/B_journal/update_journal');
            $data['excel_url'] = site_url('Report/B_report/excel_laporan_petugas');

            // $data['get_coa_a'] = $this->param_model->get_coa_a();
            // $data['dropdown_coa_e'] = $this->param_model->get_dropdown_coa_e();
            //$where_users = array('id_backend_users' => $id_backend_users);
            $data['data_users'] = $this->users_model->get_users_by_id($access_user);
            $data['data_laporan_user'] = $this->report_model->get_laporan_by_id_users(null,$access_user);
            $data['data_laporan'] = $this->report_model->get_laporan_list_report_admin(null,$access_user,@$start_date,@$end_date);
            $this->form_validation->set_rules("id_backend_groups", "Hak Akses", "trim");
            $this->form_validation->set_rules("users_access_status", "Status Akses", "trim");
        }
		...
		$this->load->view('backend-web/root/report-list', $data);
    ...
    {       
        ...
        $data['data_laporan'] = $this->report_model->get_laporan_list_report_admin(null,$start_date,$end_date);
        $this->load->view('backend-web/report/excel_laporan', $data);
    }
    ...
    {       
        ...
        $data['data_laporan'] = $this->report_model->get_laporan_list_report_admin(null,null,$start_date,$end_date);
        $this->load->view('backend-web/report/excel_laporan', $data);
    }
